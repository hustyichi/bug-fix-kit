...
当用户提供真实请求样例时，skill 会保存脱敏后的原始样例，并尽力从 curl/request 中沉淀 method、URL/path、headers、JSON body，以及 `input[0].content[0].text` 这类 JSON 字符串内层 payload。敏感 header 值写入 Markdown 前会替换为 `${ENV_NAME}` 占位符。
