...
MVP 不提供 PyPI/release 自动化、不内置 demo HTTP app、不提供 Web UI、不自动 mock 外部依赖。

---

## 2. 分发与安装

本项目是本地 Codex plugin package：
...
CLI 不提供 `diagnose`、`fix`、`status`、`verify`、`auto`。这些不是漏实现，而是边界约束：诊断和修复由 Codex skill 执行。
...
- `request.json`、`response.json`、`output.log` 由 helper CLI 写入。
- `diagnosis.md`、`fix.md` 由 Codex skills 写入。

---

## 5. `$bfk-init` / `bfk init-project`
...
当前 `PROJECT.md` 格式由 helper 生成，包含：
...
2. 执行 `bfk init-project`，写入 base URL、log file、`Content-Type`、`Authorization`。
3. 执行 `bfk new "login failed" account=13900000000 mode=e2e`。
4. 执行 `bfk run --timeout 3`。
5. 验证：
   - mock 服务收到 `POST /`；
...
- CLI 命令边界保持 `install/init-project/new/run/doctor`。
