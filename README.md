...
Bug Fix Kit（`bfk`）是一个本地 Codex 插件，帮你把一次本地服务 bug 处理成清晰流程：

```text
采集证据 -> 定位根因 -> 讨论修复方案 -> 最小修复
```
...
如果 `$bfk-locate` 因为缺少关键日志只能输出 `unknown`，可以让 BFK 自动插入临时探针日志补证据：
...
- 根据 `root-cause.md` 中的缺失证据，在相关代码处插入少量带 `BFK-PROBE` 标记的日志行（每轮最多 5 条，最多 2 轮，不打印密码、token 等敏感信息）；
- 重放同一请求，把含探针的新日志刷新到 `.bfk/output.log`；
- 如果探针日志没有出现（服务可能未重载），会提示你重启服务，而不会基于缺失的日志推理。

之后重新运行 `$bfk-locate`。定位完成后，用一条命令完全撤销所有探针：

```text
$bfk-probe --revert
```

撤销会删除所有带 `BFK-PROBE` 标记的行（探针是独立整行，删除即精确还原），并校验零残留。探针未撤销前，新的 `$bfk-capture` 和 `$bfk-fix` 会拒绝执行并提醒你先撤销。
...
`$bfk-fix-plan` 会读取 `.bfk/root-cause.md` 和相关代码，只写当前最新版 `.bfk/fix-plan.md`，不修改代码。方案不满意时，直接反馈你的约束或偏好后继续运行 `$bfk-fix-plan`，它会重写为新的当前方案。
...
├── request.json     # 本次实际请求
├── response.json    # 本次响应
├── output.log       # 本次执行窗口内新增日志（probe 重放后包含探针日志）
├── root-cause.md    # locate 生成的根因报告
├── probe.json       # probe 会话状态（使用 $bfk-probe 时）
├── fix-plan.md      # 当前最新版修复方案
├── fix.md           # fix 生成的修复记录
...
不会。每条探针都带唯一标记 `BFK-PROBE`，`$bfk-probe --revert` 会完全撤销并校验零残留。在探针撤销前，新的 `$bfk-capture` 和 `$bfk-fix` 会拒绝执行，避免探针被遗忘或误提交。
