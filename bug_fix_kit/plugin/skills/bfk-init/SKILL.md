...
2. When the user provides a real curl/request sample, preserve it through the helper so `.bfk/PROJECT.md` contains the raw sample, request contract, parameter contract, and repository evidence.
...
5. Preserve request headers that generated runners should send, including local auth values needed for replay. `.bfk/` is gitignored local workspace state; use env placeholders only when the user explicitly asks for them. `auth-note` style content is documentation only.
